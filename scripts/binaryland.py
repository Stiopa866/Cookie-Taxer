...
if __name__ == "__main__":
		cond = 'check_variable = {{ which = twa_{i} value = %s }}'
		body = 'set_variable = {{ which = carry value = %s }}\nsubtract_variable = {{ which = twa_{i} value = SAO }}'
		form = 'if = {{\n\t\tlimit = {{\n\t\t\t%s\n\t\t}}\n\t\t%s\n\t}}\n\telse = {{\n\t\t%s\n\t}}' % (cond, '%s', '%s')

		with open('output.txt', 'w') as f:
			f.write(btree([i for i in range(0, 10)], form, body))
